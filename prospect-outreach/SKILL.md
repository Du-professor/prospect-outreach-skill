...
description: >
  ...
  outbound sales, partner development, or gradual follow-up sequences. Inputs
  may include the user's company website or description, prospect websites or
  descriptions, contact names/roles, target market, preferred language, and
...
Chinese user intents that should trigger this workflow include: 潜在客户分析, 客户匹配, 外联邮件, 拓客邮件, and 销售邮件草稿.
...
2. **Research current facts.** When URLs, company names, or public claims are provided, gather current public information using available web tools. Separate sourced facts from interpretation. If browsing is unavailable, use only the provided material and state that limitation.
...
### Email Package
Provide both a **Prospect-local language version** and an **English version**. For each email in each language, provide:
- Subject
- Body
